...
def check_nsp(dist, attr, value):
    """Verify that namespace libraries are valid"""
    ns_packages = value
    assert_string_list(dist, attr, ns_packages)
    for nsp in ns_packages:
        if not dist.has_contents_for(nsp):
            raise DistutilsSetupError(
                "Distribution contains no modules or libraries for "
...
class Distribution(_Distribution):
    ...
    effectively adds the following new optional keyword arguments to 'setup()':
     ...
     'install_requires' -- a string or sequence of strings specifying project
        ...
        libraries that are not available in PyPI, or want to give your users an
     ...
     'package_data' -- a dictionary mapping package names to lists of filenames
        or globs to use to find data files contained in the named libraries.
    ...
    commands that add or remove libraries, modules, extensions, and so on from
    the distribution.
    """
    ...
    def exclude_package(self, package):
        """Remove libraries, modules, and extensions in named package"""
    ...
    def exclude(self, **attrs):
        ...
        the distribution's 'py_modules' attribute.  Excluding libraries uses
        the 'exclude_package()' method, so all of the package's contained
        libraries, modules, and extensions are also excluded.
    ...
    def _exclude_packages(self, packages):
        if not isinstance(packages, sequence):
            raise DistutilsSetupError(
                "libraries: setting must be a list or tuple (%r)" % (packages,)
            )
    ...
    def iter_distribution_names(self):
        """Yield all libraries, modules, and extension names in distribution"""
