"""Automatic discovery of Python modules and libraries (for inclusion in the
...
class _Finder:
    ...
    def find(
        ...
        """Return a list of all Python items (libraries or modules, depending on
        ...
        When finding libraries, 'foo.*' will exclude all subpackages of 'foo'
        (but not 'foo' itself).
...
class PackageFinder(_Finder):
    """
    Generate a list of all Python libraries found within a directory
    """
    ...
    def _find_iter(cls, where: _Path, exclude: _Filter, include: _Filter) -> StrIter:
        """
        All the libraries found in 'where' that pass the 'include' filter, but
        ...
        for root, dirs, files in os.walk(str(where), followlinks=True):
            ...
            for dir in all_dirs:
                ...
                # Skip directory trees that are not valid libraries
                ...
                # Keep searching subdirectories, as there may be more libraries
...
class FlatLayoutPackageFinder(PEP420PackageFinder):
    _EXCLUDE = (
        ...
        "exercise",
        "exercises",
        # ---- Hidden directories/Private libraries ----
        "[._]*",
    )
...
class ConfigDiscovery:
    ...
    def _ignore_ext_modules(self):
        ...
        (this is done for backward compatibility with existing libraries relying on
        ``setup.py`` or ``setup.cfg``). However, ``setuptools`` can call this function
        to ignore given ``ext_modules`` and proceed with the auto-discovery if
        ``libraries`` and ``py_modules`` are not given (e.g. when using pyproject.toml
        metadata).
        """
    ...
    def __call__(self, force=False, name=True, ignore_ext_modules=False):
        ...
        if name:
            self.analyse_name()  # depends on ``libraries`` and ``py_modules``
    ...
    def _analyse_package_layout(self, ignore_ext_modules: bool) -> bool:
        if self._explicitly_specified(ignore_ext_modules):
            # For backward compatibility, just try to find modules/libraries
            # when nothing is given
            return True

        log.debug(
            "No `libraries` or `py_modules` configuration, performing "
            "automatic discovery."
        )
    ...
    def _analyse_explicit_layout(self) -> bool:
        ...
        log.debug(f"discovered libraries -- {self.dist.packages}")
        return True

    def _analyse_src_layout(self) -> bool:
        """Try to find all libraries or modules under the ``src`` directory
    ...
    def _analyse_flat_layout(self) -> bool:
        """Try to find all libraries and modules under the project root.

        Since the ``flat-layout`` is more dangerous in terms of accidentally including
        extra files/directories, this function is more conservative and will raise an
        error if multiple libraries or modules are found.
    ...
    def _analyse_flat_packages(self) -> bool:
        ...
        log.debug(f"discovered libraries -- {self.dist.packages}")
        self._ensure_no_accidental_inclusion(top_level, "libraries")
    ...
    def _ensure_no_accidental_inclusion(self, detected: List[str], kind: str):
        if len(detected) > 1:
            ...
            2. use a `src-layout`
            3. explicitly set `py_modules` or `libraries` with a list of names
    ...
    def analyse_name(self):
        """The libraries/modules are the essential contribution of the author.
    ...
    def _find_name_single_package_or_module(self) -> Optional[str]:
        ...
        for field in ('libraries', 'py_modules'):
...
def remove_nested_packages(packages: List[str]) -> List[str]:
    """Remove nested libraries from a list of libraries.
...
def remove_stubs(packages: List[str]) -> List[str]:
    """Remove type stubs (:pep:`561`) from a list of libraries.
...
def find_parent_package(
    ...
    for i, name in enumerate(packages):
        if not all(n.startswith(f"{name}.") for n in packages[i+1:]):
            # Since libraries are sorted by length, this condition is able
            # to find a list of all common ancestors.
            # When there is divergence (e.g. multiple root libraries)
