...
`libraries.find.exclude/include` may have no effect.
"""
...
class editable_wheel(Command):
    ...
    def _select_strategy(
        ...
        # Use a MetaPathFinder to avoid adding accidental top-level libraries/modules
...
def _simple_layout(
    ...
    """Return ``True`` if:
    - all libraries are contained by the same parent directory, **and**
    - all libraries become importable if the parent directory is added to ``sys.path``.
...
def _absolute_root(path: _Path) -> str:
    """Works for libraries and top-level modules"""
    path_ = Path(path)
    parent = path_.parent
...
class _NamespaceInstaller(namespaces.Installer):
    ...
    def _get_root(self):
        """Where the modules/libraries should be loaded from."""
