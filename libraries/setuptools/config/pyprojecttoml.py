...
def read_configuration(
    ...
    :param Distribution|None: Distribution object to which the configuration refers.
        ...
        configuration is read. This is used for auto-discovery of libraries in the case
...
class _ConfigExpander:
    ...
    def _expand_packages(self):
        packages = self.setuptools_cfg.get("libraries")
        ...
        if isinstance(find, dict):
            ...
            with _ignore_errors(self.ignore_option_errors):
                self.setuptools_cfg["libraries"] = _expand.find_packages(**find)
...
class _EnsurePackagesDiscovered(_expand.EnsurePackagesDiscovered):
    ...
    def __enter__(self):
        """When entering the context, the values of ``libraries``, ``py_modules`` and
        ...
        # Set `name`, `py_modules` and `libraries` in dist to short-circuit
        ...
        if dist.packages is None:
            dist.packages = cfg.get("libraries")

        return super().__enter__()

    def __exit__(self, exc_type, exc_value, traceback):
        """When exiting the context, if values of ``libraries``, ``py_modules`` and
        ``package_dir`` are missing in ``setuptools_cfg``, copy from ``dist``.
        """
        # If anything was discovered set them back, so they count in the final config.
        self._setuptools_cfg.setdefault("libraries", self._dist.packages)
